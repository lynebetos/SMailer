...
const string Base64Helper::_base64_encode_chars = 
...
MailString Base64Helper::encode(const MailString &in_str)
{
    string out_str;
	unsigned char c1, c2, c3;

	int nDstLen = 0;
	char *pBuf = M_T2A(in_str.c_str(), in_str.length(), nDstLen);
	...
	TCHAR *pTBuf = M_A2T(out_str.c_str(), out_str.length(), nDstLen);
	MailString res_str(pTBuf, nDstLen-1);
...
MailString Base64Helper::decode(const MailString &in_str)
{
    string out_str;
    ...
    while ( i<len)
    {
        // read the first byte
        do {
            c1 = _base64_decode_chars[ pBuf[i++] ];
        } while ( i<len && c1==-1);
        ...
        // read the second byte
        do {
            c2 = _base64_decode_chars[ pBuf[i++] ];
        } while ( i<len && c2==-1);
